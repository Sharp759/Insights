...
document.getElementById('btnHome').addEventListener('click', function() {
    ...
    // If no data is imported, show welcome screen
    if (!excelData) {
        showWelcomeScreen();
    } else {
        // Just clear the charts but keep welcome screen hidden
        showToast('success', "Cleared charts - Data is still loaded");
    }
});
...
/************************************
 * CHART RENDERING FUNCTION (Updated with icons)
 ************************************/
function renderChart(type, data, options, containerId, iconClass = 'fas fa-chart-bar') {
    ...
    // Remove the duplicate title from options
    const newOptions = {...options};
    newOptions.plugins.title.display = false;
    
    new Chart(canvas, { type, data, options: newOptions });
}
/************************************
